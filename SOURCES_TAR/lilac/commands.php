...
<?php
	if(isset($command) || isset($_GET['command_add'])) {
		...
		?>
			...
			?>
			<b>Command Name:</b><br />
			<input type="text" size="40" name="command_manage[command_name]" value="<?php echo isset($command) ? rgm_esc($command->getName()) : '';?>">
			<?php echo $lilac->element_desc("command_name", "nagios_commands_desc"); ?><br />
			<br />
			<b>Command Line:</b><br />
			<textarea cols="100" rows=4 name="command_manage[command_line]"><?php echo isset($command) ? rgm_esc($command->getLine()) : '';?></textarea>
			<?php echo $lilac->element_desc("command_line", "nagios_commands_desc"); ?><br />
			<br />
			<b>Command Description:</b><br />
			<input type="text" size="100" name="command_manage[command_desc]" value="<?php echo isset($command) ? rgm_esc($command->getDescription()): '';?>">
		...
		if($numOfCommands) {
			...
			for($counter = 0; $counter < $numOfCommands; $counter++) {
				...
				?>
				<td height="20" class="altLeft" onclick="checkLine('line<?php echo $counter?>','check<?php echo $counter?>');">&nbsp;<a href="commands.php?command_id=<?php echo $command_list[$counter]->getId();?>"><?php echo rgm_esc($command_list[$counter]->getName());?></a></td>
				<td height="20" class="altLeft" onclick="checkLine('line<?php echo $counter?>','check<?php echo $counter?>');">&nbsp;<?php echo rgm_esc($command_list[$counter]->getDescription());?></td>
				<td align="center"><input type="checkbox" id="check<?php echo $counter?>" class="checkbox" name="EoN_Actions_Checks_Command[]" value="<?php echo rgm_esc($command_list[$counter]->getId());?>" onclick="checkBox('line<?php echo $counter?>','check<?php echo $counter?>');"></td>
				</tr>
				<?php
			}
			?>
