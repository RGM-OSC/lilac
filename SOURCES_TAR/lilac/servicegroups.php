...
<?php
	if(isset($_GET['id'])) {
		...
		if($_GET['section'] == 'general') {
			...
			<tr>
				...
				if(isset($_GET['edit'])) {
					...
					<form name="servicegroup_form" method="post" action="servicegroups.php?id=<?php echo $_GET['id'];?>&section=general&edit=1">
						...
						<b>Service Group Name:</b> <input type="text" name="servicegroup_name" value="<?php echo rgm_esc($serviceGroup->getName());?>">
						<?php echo $lilac->element_desc("servicegroup_name", "nagios_servicegroups_desc"); ?><br />
						<br />
						<b>Description:</b><br />
						<input type="text" size="80" name="alias" value="<?php echo rgm_esc($serviceGroup->getAlias());?>">
			...
			<?php
					...
					?>
					<b>Service Group Name:</b> <?php echo rgm_esc($serviceGroup->getName());?><br />
					<b>Description:</b> <?php echo rgm_esc($serviceGroup->getAlias());?><br />
					<br />
			...
			<tr>
				...
				if(isset($_GET['edit'])) {
					...
					<form name="command_form" method="post" action="servicegroups.php?id=<?php echo $_GET['id'];?>&section=extended&edit=1">
						...
						<b>Notes:</b> <input type="text" name="notes" value="<?php echo rgm_esc($serviceGroup->getNotes());?>">
						<?php echo $lilac->element_desc("notes", "nagios_servicegroups_desc"); ?><br />
						<br />
						<b>Notes URL:</b> <input type="text" name="notes_url" value="<?php echo rgm_esc($serviceGroup->getNotesUrl());?>">
						<?php echo $lilac->element_desc("notes", "nagios_servicegroups_desc"); ?><br />
						<br />
						<b>Action URL:</b> <input type="text" name="action_url" value="<?php echo rgm_esc($serviceGroup->getActionUrl());?>">
						<?php echo $lilac->element_desc("notes", "nagios_servicegroups_desc"); ?><br />
						<br />
						<br />
					...
					if($serviceGroup->getNotes() != '') {
						?>
						<b>Notes:</b> <?php echo rgm_esc($serviceGroup->getNotes());?><br />
						<?php
					}
					if($serviceGroup->getNotesUrl() != '') {
						?>
							<b>Notes URL:</b> <?php echo rgm_esc($serviceGroup->getNotesUrl());?><br />
						<?php
					}
					if($serviceGroup->getActionUrl() != '') {
						?>
						<b>Action URL:</b> <?php echo rgm_esc($serviceGroup->getActionUrl());?><br />
						<?php
					}
					?>
					<br />
			...
			<tr>
				...
				<table width="100%" align="center" cellspacing="0" cellpadding="2" border="0">
					...
					for($counter = 0; $counter < $numOfMembers; $counter++) {
						...
						<td height="20" class="altRight"><b><?php echo rgm_esc($text);?></b></td>
						</tr>
						<?php
					}
					?>
	...
	if(!isset($_GET['servicegroup_add'])) {	
		...
		if($numOfServiceGroups) {
			...
			<td>Group Name</td>
			<td>Description</td>
			<td align="center"><a href="#" class="btn btn-primary btn-xs" onClick="checkUncheckAll('EoN_Actions_Checks_ServiceGroup');">ALL</a></td>
			</tr>
			...
			for($counter = 0; $counter < $numOfServiceGroups; $counter++) {
				...
				?>
				<td height="20" class="altLeft" onclick="checkLine('line<?php echo $counter?>','check<?php echo $counter?>');">&nbsp;<a href="servicegroups.php?id=<?php echo $servicegroups_list[$counter]->getId();?>"><?php echo rgm_esc($servicegroups_list[$counter]->getName());?></a></td>
				<td height="20" class="altRight" onclick="checkLine('line<?php echo $counter?>','check<?php echo $counter?>');"><?php echo rgm_esc($servicegroups_list[$counter]->getAlias());?></td>
