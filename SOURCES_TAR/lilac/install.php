			...
			if(!$error && $mysqlPopulate) {
				...
				if(!$dbConn) {
					...
					if(!mysql_select_db($mysqlDatabase, $dbConn)) {
						...
						// Load the data
						exec("/usr/bin/mysql -h " . $mysqlHostname . " -u " . $mysqlUsername . " -p" . $mysqlPassword . " " . $mysqlDatabase . " < " . dirname(__FILE__) . "/sqldata/schema.sql", $output, $retVal);
						if($retVal != 0) {
							$error = "Failed to import database schema. Make sure the mysql binary is in the search path for the web user.";
						}
						else {
							// Import labels
							exec("/usr/bin/mysql -h " . $mysqlHostname . " -u " . $mysqlUsername . " -p" . $mysqlPassword . " " . $mysqlDatabase . " < " . dirname(__FILE__) . "/sqldata/lilac-nagios-en-label.sql", $output, $retVal);
							if($retVal != 0) {
								$error = "Failed to import Nagios labels.  Error was: <br />" . str_replace("\n", "<br />", $output[count($output)]);
							}
							else {
								// Import Seed
								exec("/usr/bin/mysql -h " . $mysqlHostname . " -u " . $mysqlUsername . " -p" . $mysqlPassword . " " . $mysqlDatabase . " < " . dirname(__FILE__) . "/sqldata/seed.sql", $output, $retVal);
								if($retVal != 0) {
...
if($stage == 1) {
	...
	exec("/usr/bin/which mysql", $output, $retVal);
	if($retVal != 0) {
		$fail = true;
	}
	else {
	...
	exec("/usr/bin/which nmap", $output, $retVal);
	if($retVal != 0) {
		$fail = true;
	}
	else {
	...
	$cliOutput = array();
	$retVal = '';
	$result = exec("/usr/bin/php " . dirname(__FILE__) . "/install.php", $cliOutput, $retVal);
	$fail = $retVal != 0;
