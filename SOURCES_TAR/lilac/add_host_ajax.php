	...
	if ($_POST['id'] == "null"){
		...
		if(!count($templateList)) {
			...
			foreach($templateList as $template) {
				echo "<option value='".$template->getId()."'>".rgm_esc($template->getName())."</option>";
			}
			echo "</select>&nbsp;";
		...
		foreach($_SESSION['templates'] as $template){
			foreach( $templateList as $temp){
				if ( $template == $temp->getId()){
		        	...
		        	echo "<td height='20' class='altRight'><b>".rgm_esc($temp->getName())."</b></td></tr>";
		        	$i++;
		        	break;
	        	}
        	}
		...
		if ( $_POST['option'] != "delete"){
	        ...
	        foreach ($templateList as $template){
	        	...
	        	if ($template->getId() == $_POST['id']){
	        		...
	        		echo "<td height='20' class='altRight'><b>".rgm_esc($template->getName())."</b></td></tr>";
	        		break;
				}
	        }
	    }
