			...
			<tr>
				...
				if(isset($_GET['edit'])) {	// We're editing checks information
					...
					<form name="command_form" method="post" action="contactgroups.php?contactgroup_id=<?php echo $_GET['contactgroup_id'];?>&edit=1">
						...
						<b>Contact Group Name:</b> <input type="text" name="contactgroup_manage[contactgroup_name]" value="<?php echo rgm_esc($contactGroupInfo->getName());?>">
						<?php echo $lilac->element_desc("contactgroup_name", "nagios_contactgroups_desc"); ?><br />
						<br />
						<b>Description:</b><br />
						<input type="text" size="80" name="contactgroup_manage[alias]" value="<?php echo rgm_esc($contactGroupInfo->getAlias());?>">
					...
					?>
					<b>Contact Group Name:</b> <?php echo rgm_esc($contactGroupInfo->getName());?><br />
					<b>Description:</b> <?php echo rgm_esc($contactGroupInfo->getAlias());?><br />
					<br />
			...
			<tr>
				...
				<table width="100%" align="center" cellspacing="0" cellpadding="2" border="0">
					...
					for($counter = 0; $counter < $numOfMembers; $counter++) {
						...
						<td height="20" class="altRight"><b><?php echo rgm_esc($member_list[$counter]->getNagiosContact()->getName());?>:</b> <?php echo rgm_esc($member_list[$counter]->getNagiosContact()->getAlias());?></td>
						</tr>
						<?php
					}
					?>
				...
				<input type="hidden" name="contactgroup_manage[member_add][contactgroup_id]" value="<?php echo rgm_esc($_GET['contactgroup_id']);?>" />
	...
	if(!isset($_GET['contactgroup_add'])) {	
		...
		if($numOfContactGroups) {
			...
			for($counter = 0; $counter < $numOfContactGroups; $counter++) {
				...
				?>
				<td height="20" class="altLeft" onclick="checkLine('line<?php echo $counter?>','check<?php echo $counter?>');">&nbsp;<a href="contactgroups.php?contactgroup_id=<?php echo $contactgroups_list[$counter]->getId();?>"><?php echo rgm_esc($contactgroups_list[$counter]->getName());?></a></td>
				<td height="20" class="altRight" onclick="checkLine('line<?php echo $counter?>','check<?php echo $counter?>');"><?php echo rgm_esc($contactgroups_list[$counter]->getAlias());?></td>
				<td align="center"><input type="checkbox" id="check<?php echo $counter?>" class="checkbox" name="EoN_Actions_Checks_ContactGroup[]" value="<?php echo rgm_esc($contactgroups_list[$counter]->getId());?>" onclick="checkBox('line<?php echo $counter?>','check<?php echo $counter?>');"></td>
				</tr>
				<?php
			}
			?>
