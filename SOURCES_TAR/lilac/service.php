...
if($service->getNagiosHostTemplate()) {
	$subtitle = " for Host Template " . rgm_esc($service->getNagiosHostTemplate()->getName());
	$sublinktitle = "Back To Host Template " . rgm_esc($service->getNagiosHostTemplate()->getName());
	$sublink = "host_template.php?id=" . $service->getNagiosHostTemplate()->getId() . "&section=services";
}
else if($service->getNagiosHost()) {
	$subtitle = " for Host " . rgm_esc($service->getNagiosHost()->getName());
	$sublinktitle = "Back To Host " . rgm_esc($service->getNagiosHost()->getName());
	$sublink = "hosts.php?id=" . $service->getNagiosHost()->getId() . "&section=services";
}
else if($service->getNagiosHostgroup()) {
	$subtitle = " for Hostgroup " . rgm_esc($service->getNagiosHostgroup()->getName());
	$sublinktitle = "Back To Hostgroup " . rgm_esc($service->getNagiosHostgroup()->getName());
	...
	print_window_header("Service Info for " . $service->getDescription() . $subtitle, "100%");	
		...
		if($_GET['section'] == 'general') {
			...
			<tr>
				...
				if(isset($_GET['edit'])) {	// We're editing general information
					...
					<b>Description:</b><br />
					<input type="text" size="80" name="service_manage[service_description]" value="<?php echo rgm_esc($service->getDescription());?>">
					<?php echo $lilac->element_desc("service_description", "nagios_service_template_desc"); ?><br />
					<br />
                    <b>Display Name: (Optional)</b><br />
                    <input type="text" size="80" name="service_manage[display_name]" value="<?php echo rgm_esc($service->getDisplayName());?>"><br />
					...
					?>
					<b>Description:</b> <?php echo rgm_esc($service->getDescription());?><br />
                    <?php
                    $displayName = $service->getDisplayName();
                    if(!empty($displayName)) {
                     ?>
                      <b>Display Name:</b> <?php echo rgm_esc($service->getDisplayName());?><br />
                     <?php
                    }
                    ?>
					<br />
			...
			<tr>
				...
				<table width="100%" align="center" cellspacing="0" cellpadding="2" border="0">
					...
					for($counter = 0; $counter < $numOfTemplates; $counter++) {
						...
						<td height="20" class="altRight"><b><?php echo rgm_esc($templateInheritances[$counter]->getName());?></b></td>
						</tr>
						<?php
					}
					?>
		...
		if($_GET['section'] == 'notifications') {
			...
			<tr>
				...
				if(isset($_GET['edit'])) {	// We're editing general information
					...
					elseif(isset($serviceValues['notification_on_warning'])) {
						...
						print("<b> - Inherited From: </b><i>".rgm_esc($serviceValues['notification_on_warning']['source']['name'])."</i>");
						print("<br />");
					}
					...
					elseif(isset($serviceValues['stalking_on_ok'])) {
						...
						print("<b> - Inherited From: </b><i>".rgm_esc($serviceValues['stalking_on_ok']['source']['name'])."</i>");
						print("<br />");
					}					
					?>
					<br />
			...
			?>
				...
				<td height="20" class="altRight"><?php echo rgm_esc($cmdObj['command']['command']->getDescription());?></td>
                                </tr>
				</table>
				<br><br>
			<?php
				...
				<td valign="top">
							...
							<table width="100%" align="center" cellspacing="0" cellpadding="2" border="0">
								...
								if(count($inherited_list)) {
									...
									foreach($inherited_list as $servicegroup) {
										...
										?>
										<td height="20" width="80" nowrap="nowrap" class="altLeft">&nbsp;</td>
										<td height="20" class="altRight"><b><?php echo rgm_esc($servicegroup->getName());?>:</b> <?php echo rgm_esc($servicegroup->getAlias());?></td>
										</tr>
										<?php
										$counter++;
									}
						...
						<table width="100%" align="center" cellspacing="0" cellpadding="2" border="0">
							...
							for($counter = 0; $counter < $numOfServiceGroups; $counter++) {
								...
								<td height="20" class="altRight"><b><?php echo rgm_esc($servicegroups_list[$counter]->getNagiosServiceGroup()->getName());?>:</b> <?php echo rgm_esc($servicegroups_list[$counter]->getNagiosServiceGroup()->getAlias());?></td>
								</tr>
								<?php
							}
							?>
					...
					<td>
							...
							<table width="100%" align="center" cellspacing="0" cellpadding="2" border="0">
								...
								if(count($inherited_list)) {
									...
									foreach($inherited_list as $customObjectVariable) {
										...
										?>
										<td height="20" width="80" nowrap="nowrap" class="altLeft">&nbsp;</td>
										<td height="20" class="altRight"><b>$_SERVICE<?php echo rgm_esc($customObjectVariable->getVarName());?>$:</b> <?php echo rgm_esc($customObjectVariable->getVarValue());?> from <strong>(service template) <?php echo rgm_esc($customObjectVariable->getNagiosServiceTemplate()->getName()); ?></strong></td>
										</tr>
										<?php
						...
						<table width="100%" align="center" cellspacing="0" cellpadding="2" border="0">
							...
							foreach($customObjectVariables as $customObjectVariable) {
								...
								<td height="20" class="altRight"><b>$_SERVICE<?php echo rgm_esc($customObjectVariable->getVarName());?>$:</b> <?php echo rgm_esc($customObjectVariable->getVarValue());?></td>
								</tr>
								<?php
